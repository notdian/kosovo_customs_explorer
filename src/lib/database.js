...
function buildTreeFromList(list) {
    ...
    for (const node of byId.values()) if (node.subRows?.length) node.subRows.sort(sortChildren);
...
export class CustomsDataService {
     ...
     */
    static async searchByFields(idPrefix = '', descQuery = '', { parentLimit = 200, itemLimit = 4000 } = {}) {
        try {
            ...
            if (idP) {
                const [byCode, byTerms] = await Promise.all([
                    db.parents.where('codeLC').startsWith(idP).limit(parentLimit * 2).toArray(),
                    db.parents.where('terms').startsWith(idP).limit(parentLimit * 4).toArray(),
                ]);
                const map = new Map();
            ...
            if (desc) {
                ...
                if (tokens.length) {
                    ...
                    for (const t of tokens) {
                        const rs = await db.parents.where('terms').startsWith(t).limit(parentLimit * 4).toArray();
                        perToken.push(rs);
                    }
                    rootsByDesc =
                        perToken.length > 1
                            ? intersectById(perToken.map((arr) => arr.map((r) => ({ id: r.rootId }))))
