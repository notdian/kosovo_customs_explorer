...
function highlightMatches(text, query) {
    ...
    while ((match = regex.exec(t)) !== null) {
        ...
        parts.push(<span key={parts.length} className="bg-amber-200 rounded px-0.5">{match[0]}</span>);
...
function VirtualizedTreeTable({
    ...
    const minTableWidth = 170 + gridTemplate.matchAll(/(\d+)px/g).map(x => x[1]).reduce((a, b) => a + Number(b), 0);
                        ...
                        <div style={{ height: totalSize, position: 'relative' }}>
                            {loading ? (
                                <div
                                    ...
                                    style={{ gridTemplateColumns: gridTemplate, position: 'absolute', top: 0, left: 0, right: 0, height: 56 }}
                                >
...
function App() {
        ...
        () => [
                ...
                cell: (info) => {
                    ...
                    return (
                        <div className="flex items-start gap-2 min-w-0">
                            ...
                            <span className="font-mono truncate" title={`#${info.row.original.id}`}>
            ...
            },
            {
                ...
                cell: (info) => {
                    ...
                    return (
                        <div className="break-words hyphens-auto truncate overflow-hidden" lang="sq" title={v}>
                            {highlightMatches(v, q)}
                        </div>
                    );
                },
            },
            { header: 'Bazë', accessorKey: 'percentage', cell: (i) => <span>{formatPercent(i.getValue())}</span> },
            { header: 'CEFTA', accessorKey: 'cefta', cell: (i) => <span>{formatPercent(i.getValue())}</span> },
            { header: 'MSA', accessorKey: 'msa', cell: (i) => <span>{formatPercent(i.getValue())}</span> },
            { header: 'TRMTL', accessorKey: 'trmtl', cell: (i) => <span>{formatPercent(i.getValue())}</span> },
            { header: 'TVSH', accessorKey: 'tvsh', cell: (i) => <span>{formatPercent(i.getValue())}</span> },
            { header: 'Aksizë', accessorKey: 'excise', cell: (i) => <span>{formatPercent(i.getValue())}</span> },
            {
    ...
    return (
        <div className="min-h-screen bg-background p-4 sm:p-6">
            <h1 className="text-xl font-bold mb-4">Shfletuesi i të Tarifave Doganore të Republikës së Kosovës</h1>
            <aside className="mb-4 text-xs sm:text-sm border rounded-lg p-3 bg-amber-50 border-amber-200">
                ...
                <p className="mb-1">
                    Ky aplikacion është jo-zyrtar dhe <strong>nuk përfaqëson Doganën e Kosovës</strong>. Të dhënat ngarkohen nga burime publike dhe mund të jenë të papërditësuara. Për informata zyrtare, referojuni publikimeve zyrtare.
                </p>
            </aside>
            {/* Search + Price */}
            <div className="mb-4 grid grid-cols-1 md:grid-cols-5 gap-3">
                <div>
                    <label className="block text-xs text-gray-600 mb-1">Prefiksi i Kodit / ID-së</label>
                    <input
                        type="text"
                        value={idQuery}
                ...
                <div>
                    <label className="block text-xs text-gray-600 mb-1">Çmimi i produktit</label>
                    <input
                        type="number"
                        inputMode="decimal"
                        step="0.01"
